<?php

namespace Modules\Permission\Transformers\V1\Admin;

use Illuminate\Http\Resources\Json\JsonResource;
use Illuminate\Support\Collection;
use Modules\Permission\Enums\Roles;

class AdminRoleResource extends JsonResource
{
    /**
     * Transform the resource into an array.
     *
     * @param \Illuminate\Http\Request
     * @return array
     */
    public function toArray($request)
    {
        return collect($this->resource)->when($this->whenAppended('name'), function (Collection $collection) {
            $collection->put('is_super_admin', $this->resource->name == Roles::SUPER_ADMIN['name_en']);
        })->when($this->resource->relationLoaded('permissions'), function (Collection $collection) {
            $collection->put('permissions', AdminPermissionResource::collection($this->resource->permissions));
        });
    }
}
