...
namespace Modules\Poster\Transformers\Api\Admin;

use Illuminate\Http\Resources\Json\JsonResource;
use Illuminate\Support\Collection;
use Modules\Poster\Enums\PosterStatus;

class ApiAdminPosterResource extends JsonResource
{
    /**
    ...
    public function toArray($request)
    {
        return collect($this->resource)->when(array_key_exists('created_at', $this->resource->getAttributes()), function (Collection $collection) {
            ...
            $collection->put('status_translated', PosterStatus::getDescription($this->resource->status));
        });
    }
}
